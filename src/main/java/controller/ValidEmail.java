...
public class ValidEmail {
        ...
        if (this.code.getText().equals(this.codeValid) && !this.codeValid.equals("emailIncorrect")){
            if (this.resetMdp){
                if (!this.utilisateur.isValid()){UtilisateurController.valid(this.utilisateur);
                    Main.changeScene("EditProfil", new EditProfil(this.utilisateur), "Modifions notre profil !!");
                }
            } else {
                UtilisateurController.valid(this.utilisateur);
                Main.changeScene("Accueil", new Accueil(), "Bienvenue " + this.utilisateur.getPrenom());
            }
    ...
    @FXML void switchConnexion(ActionEvent event) {Main.changeScene("Connexion", new ConnexionController(), "Connectez vous ;)");}
}
